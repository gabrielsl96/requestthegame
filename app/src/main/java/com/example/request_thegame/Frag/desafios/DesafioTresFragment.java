...
public class DesafioTresFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(desafio.getStatusDesafios().getStatusDesafioDois().equals("Concluído") &&
        ...
        else if(desafio.getStatusDesafios().getStatusDesafioTres().equals("Indisponível")){
            view = inflater.inflate(R.layout.view_desafio_bloqueado_anterior,container,false);

        }

        else if(desafio.getStatusDesafios().getStatusDesafioTres().equals("Executando")){
            ...
            textTituloConcluido.setText("Terceiro desafio concluído");

            if (desafio.getMensagemFinal().getTipoMensagem().equals("Código Morse")) {
                textMensagemFinal.setText(TradutorCodigoMorse.getCodigoMorse(desafio.getMensagemFinal().getTerceiraParte()));
            } else {
                textMensagemFinal.setText(desafio.getMensagemFinal().getTerceiraParte());
            }
        }

        else{
            view = inflater.inflate(R.layout.fragment_desafio_tres,container,false);
            ...
            button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    String[] idDesafio = usuario.getDesafio().split("/");
                                    reference.child("Desafios").child(idDesafio[0]).child(idDesafio[1]).child("statusDesafios").child("statusDesafio").setValue("Executando terceiro desafio");
                                    Intent intent = new Intent(getContext(), DesafioTresActivity.class);
                                    intent.putExtra("desafio",desafio.getTerceiroDesafio());
